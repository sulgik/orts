...
q = bandit.allocate(["A", "B", "C"], draw=50000, rng=rng)                    # A1 draws, A2 winner shares
...
# Section 5's two controls: decay is recognition's, acting on what is carried;
# aggressiveness is action's, acting on how strongly the belief drives traffic.
bandit.update(batch2, decay=0.1)                                          # forget 10% of the carried precision
print("gamma = 2 concentrates:", bandit.allocate(aggressive=2.0, draw=50000, rng=rng).shares)
print("with a 5% floor:", bandit.allocate(floor=0.05, draw=50000, rng=rng).shares)

# The arm set is free: drop C from the next batch and add a brand-new arm D
# (Section 6.1; once D is observed it joins the state by Supplement B's augmentation).
q = bandit.allocate(["A", "B", "D"], draw=50000, rng=rng)
print("allocate over A, B, D:", q.shares, "(D has no posterior yet and gets the uniform share)")

# Stopping-rule quantities (Supplement G) come with every allocation.
